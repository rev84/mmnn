...
Item = class Item {
  constructor(itemId1, params1) {
    ...
    this.params = params1;
    this.amount = [];
    for (level = j = 0, ref = this.params.cost.length; (0 <= ref ? j < ref : j > ref); level = 0 <= ref ? ++j : --j) {
      this.amount[level] = 0;
    }
  }
  ...
  getNameWithLevel(level) {
    if (this.getMaxLevel() - this.getMinLevel() === 0) {
      ...
      return this.getName() + ('★'.repeat(level + 1));
    }
  }
...
ItemEditor = (function() {
  class ItemEditor {
    static init(parentObject) {
      var itemId, itemObject, items, j, len, ref;
      this.parentObject = parentObject;
      this.divObject = $('<div>').attr('id', this.ID);
      items = [];
    ...
    static setItems(savedata) {
      ...
      for (itemId in ref) {
        ...
        results.push((function() {
          var j, ref1, results1;
          results1 = [];
          for (level = j = 0, ref1 = itemBody.cost.length; (0 <= ref1 ? j < ref1 : j > ref1); level = 0 <= ref1 ? ++j : --j) {
            // セーブデータに持ってる数があればセット
