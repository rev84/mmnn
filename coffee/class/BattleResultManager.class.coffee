class BattleResultManager
  ...
  # HPを減らす演出
  @animate:(characterObject, enemyObject, isCharacterOffence, hpMax, hpBase, hpTo, callback = null)->
    characterTypeImg = 
    ...
    setTimeout =>
      ...
      decrease = (restTime, nowRate, decreaseRate)=>
        ...
        @setRate nowRate
        if restTime > 0
          setTimeout decrease.bind(@, restTime, nowRate, decreaseRate), (1 / @ANIMATION_DECREASE_FPS)
        else
          setTimeout =>
            @divObject.addClass 'no_display'
            callback() if callback instanceof Function
          , @ANIMATION_WAIT_AFTER_MSEC
      decrease restTime, nowRate, decreaseRate
    , @ANIMATION_WAIT_BEFORE_MSEC
